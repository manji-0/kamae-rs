#!/usr/bin/env python3
"""Smoke-test the kamae-rs plugin package without external dependencies."""
...
SKILL_ROOT = ROOT / "skills" / "kamae-rs"
...
def check_manifest_skill_paths(errors: list[str]) -> None:
    ...
    for plugin in data.get("plugins", []):
        ...
        if plugin.get("name") == "kamae-rs" and not skills:
            fail(errors, f"{marketplace.relative_to(ROOT)}: kamae-rs entry missing skills")
        for entry in skills:
            path = (ROOT / entry).resolve()
...
CRATE_GUIDE_RE = re.compile(r"references/crate-guides/[A-Za-z0-9_-]+\.md")


def check_crate_guides(errors: list[str]) -> None:
    for path in sorted([ROOT / "rules" / "defaults" / "crate-detection.md", ROOT / "skills" / "kamae-rs" / "SKILL.md"]):
        text = path.read_text(encoding="utf-8")
        for guide in sorted(set(CRATE_GUIDE_RE.findall(text))):
...
def check_python_scripts(errors: list[str]) -> None:
    script_dirs = [ROOT / "scripts", SKILL_ROOT / "scripts"]
